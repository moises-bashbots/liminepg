...
public class HeaderLote240Santander extends RegistroCNAB
{
	...
	private FieldCNAB cnab1=new FieldCNAB(26,223,230,8,"Uso Exclusivo FEBRABAN/CNAB cnab1","","A");
	
	private FieldCNAB ocorrencias=new FieldCNAB(27,231,240,10,"Codigos das ocorrencias para Retorno","","A");
	
	private ArrayList<PagamentoTituloCobranca> pagamentos = new ArrayList<>();
	
	public HeaderLote240Santander()
	{
		super();
		this.setSize(240);
	...
	public HeaderLote240Santander(Pagador pagador, int iLote, String formaLancamento)
	{		
		super();
		this.setSize(240);
		this.setTipo("HeaderLote240");
		...
		else if(formaLancamento.toLowerCase().contains("pix"))
		{
			this.getFormaLancamento().setContent("45"); // 03 DOC/TED ; 41 TED outra titularidade; 01 credito em CC 
		}
		else if(formaLancamento.toLowerCase().contains("boletomesmobanco"))
		{
			this.getServico().setContent("03");  // Pagamento Fornecedor
			this.getFormaLancamento().setContent("30"); // 20 Pagamento de contas tributos e impostos 
		}
		else if(formaLancamento.toLowerCase().contains("boletooutrosbancos"))
		{
			this.getServico().setContent("03");  // Pagamento Fornecedor
			this.getFormaLancamento().setContent("31"); // 20 Pagamento de contas tributos e impostos 
		}
		else if(formaLancamento.toLowerCase().contains("contasetributos"))
		{
			this.getLayoutDoLote().setContent("030");
			this.getServico().setContent("22");  // Pagamento tributos e contas
			this.getFormaLancamento().setContent("11"); // Pagamento de tributos e contas 
		}		
		else
		{
			this.getFormaLancamento().setContent("01"); // TEF
		}
		this.getTipoInscricao().setContent("2"); // 01 PF, 02 PJ
		this.getNumeroInscricao().setContent(pagador.getNumeroInscricao());
		String codigoConvenio = "0033"+pagador.getDadosBancarios().getAgencia()+pagador.getDadosBancarios().getConvenio();
	...
	public ArrayList<PagamentoTituloCobranca> getPagamentos() {
		return this.pagamentos;
	}

	public void setPagamentos(ArrayList<PagamentoTituloCobranca> pagamentos) {
